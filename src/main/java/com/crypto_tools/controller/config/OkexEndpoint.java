...
//@ServerEndpoint(value = "/marketStream/okex")
//@Component
public class OkexEndpoint {
    //クライアントとのセッションで、これを通してクライアントにデータ送信
    private static Logger log = LoggerFactory.getLogger(OkexEndpoint.class);
    private static final AtomicInteger onLineCount= new AtomicInteger(0);


    /**
     * 接続出来た
     * @PathParam は変@RestController数 URI パスフラグメントをメソッド呼び出しへマップできるようにします。
     */
    @OnOpen
    public void onOpen(Session session) throws InterruptedException, IOException {
        ...
        log.info("User ID:" + userId + "がOkexEndpointに接続しています");
        int cnt = onLineCount.incrementAndGet(); // 在线数加1
        log.info("今の人数は"+cnt);
//        WebSocketDispatcher.sendOkexData(userId);
    }
