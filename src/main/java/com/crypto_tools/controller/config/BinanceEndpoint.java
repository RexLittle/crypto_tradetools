...
//@ServerEndpoint(value = "/marketStream/binance")
//@Component
public class BinanceEndpoint {
    //クライアントとのセッションで、これを通してクライアントにデータ送信
    private static Logger log = LoggerFactory.getLogger(BinanceEndpoint.class);
    ...
    public void onOpen(Session session) throws InterruptedException, IOException {
        ...
        log.info("User ID:" + userId + "がBinanceEndpointに接続しています");
        int cnt = onLineCount.incrementAndGet(); // 在线数加1
        log.info("今の人数は"+cnt);
//        WebSocketDispatcher.sendBinanceData(userId);
    }
